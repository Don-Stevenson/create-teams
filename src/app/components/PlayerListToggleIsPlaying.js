...
const PlayerListToggleIsPlaying = ({ players, onTogglePlayingThisWeek }) => {
  ...
  return (
    <ul className="grid grid-cols-[repeat(auto-fill,_minmax(12rem,_1fr))] gap-1 w-full p-2 m-2 print:hidden flex-shrink">
      {sortedPlayers.map(player => (
        <li
          key={player._id}
          className={`flex items-center justify-around max-w-80 p-[10px] border border-gray-300 rounded m-1 ${
